...
Total Accepted: 47124 Total Submissions: 233026 Difficulty: Hard
...
class Solution:
	def findSubstring(self, myString, words):
		...
		for i in xrange(w):
			...
			d=i
			j=0
			while(d<=(length-w*len(words))):
				
					nextWord=myString[d+j:d+j+w]
					while (nextWord in wordsSet):
						if ((nextWord not in usedWords)):
							usedWords[nextWord]=[1,collections.deque([d+j])]
						elif (wordsSet[nextWord] > usedWords[nextWord][0]):
							usedWords[nextWord][0]+=1
							usedWords[nextWord][1].append(d+j)
						else:
							for btwn in xrange (d,usedWords[nextWord][1][0],w):
								usedWords[myString[btwn:btwn+w]][0]-=1
								usedWords[myString[btwn:btwn+w]][1].popleft()
							shift=usedWords[nextWord][1].popleft()+w-d
							usedWords[nextWord][1].append(d+j)
							d=d+shift
							j=j-shift+w
							break
						...
						if (j/w==len(words)):
							validStarts.append(d)
							
							usedWords[myString[d:d+w]][0]-=1
							usedWords[myString[d:d+w]][1].popleft()
							d=d+w
							j=j-w
							nextWord=myString[d+j:d+j+w]
							break
...
if __name__=="__main__"	:
	...
	solution=Solution()
	print (solution.findSubstring("ckj;ljsadlkjdhuewhellodedweehelloworldhelloworld", ["hello","hello","world"]))
	print (solution.findSubstring("bdcddbababbbddcbdcddcbbaacadaadbdcacdaadbbbbad", ["bbd","dcb","dcd"]))
