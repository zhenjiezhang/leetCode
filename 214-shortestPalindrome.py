...
Total Accepted: 15623 Total Submissions: 83266 Difficulty: Hard
...
class Solution(object):
    def shortestPalindrome(self, s):
        ...
        olds=s
        s=s+'%^&'+s[::-1]
        matchNum=[0 for _ in xrange(len(s))]
        ...
        while walk<len(s):
            if s[anneal]==s[walk]:
                anneal+=1
                matchNum[walk]=anneal
                walk+=1

            elif anneal>0:
                anneal=matchNum[anneal-1]

            else:
                matchNum[walk]=0
                walk+=1

        palindromeLength=matchNum[-1]
        return olds[:palindromeLength-1:-1]+olds
